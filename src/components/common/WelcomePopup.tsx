 ...
 * 
 * Features:
 * - Different messages based on user email
...
// Fun messages based on user email patterns
const getPersonalizedMessage = (email: string, name: string) => {
    ...
    // Email-based personalized messages
    const emailLower = email.toLowerCase()

    if (emailLower.includes('admin')) {
        return {
            greeting: `${timeEmoji} ${timeGreeting}, Boss ${firstName}!`,
            message: 'Siap memimpin operasional hari ini? Dashboard Anda sudah siap! 💼',
            emoji: '👑',
            color: 'from-purple-500 to-indigo-600',
            tip: 'Cek Alert DSS untuk rekomendasi penting hari ini!'
        }
    }

    if (emailLower.includes('operator')) {
        return {
            greeting: `${timeEmoji} ${timeGreeting}, ${firstName}!`,
            message: 'Yuk, semangat kerja hari ini! Ada pesanan menunggu! 🚀',
            emoji: '💪',
            color: 'from-blue-500 to-cyan-600',
            tip: 'Quick tip: Gunakan voice order untuk input cepat!'
        }
    }

    if (emailLower.includes('test') || emailLower.includes('demo')) {
        return {
            greeting: `${timeEmoji} Halo, Tester ${firstName}!`,
            message: 'Selamat mencoba SIM4LON! Jelajahi semua fiturnya ya! 🎮',
            emoji: '🧪',
            color: 'from-green-500 to-emerald-600',
            tip: 'Coba fitur Voice Widget di pojok kanan bawah!'
        }
    }

    if (emailLower.includes('pangkalan')) {
        return {
            greeting: `${timeEmoji} ${timeGreeting}, ${firstName}!`,
            message: 'Dashboard Pangkalan Anda sudah siap! Cek stok dan catat penjualan! 📊',
            emoji: '🏪',
            color: 'from-orange-500 to-amber-600',
            tip: 'Pantau laporan harian untuk monitor keuntungan!'
        }
    }

    // Default fun messages (random)
    const defaultMessages = [
        ...
        },
        {
            ...
            tip: 'Klik pada KPI Card untuk detail lebih lanjut!'
        },
        {
    ...
    useEffect(() => {
        ...
        const showWelcome = async () => {
            try {
                ...
                setPersonalizedMsg(getPersonalizedMessage(profileData.email, profileData.name))
