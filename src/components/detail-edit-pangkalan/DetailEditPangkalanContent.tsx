...
export default function DetailEditPangkalanContent() {
  ...
  const handleSave = async (formData: Omit<Pangkalan, 'id' | 'createdAt' | 'updatedAt'>) => {
    setIsSaving(true)
    try {
      // Simulate API call
...
<div className="flex-1 space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-2">
          <a href="./daftar-pangkalan.html" className="text-muted-foreground hover:text-foreground transition-colors">
            Pangkalan
          </a>
          <SafeIcon name="ChevronRight" className="h-4 w-4 text-muted-foreground" />
          <span className="text-foreground font-semibold text-lg">{pangkalan.nama}</span>
        </div>
