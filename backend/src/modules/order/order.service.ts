 ...
 */
    ...
    async create(dto: CreateOrderDto, userId?: string) {
        ...
        if (total3kgOrdered > 0) {
            // Get pangkalan's monthly allocation
            ...
            if (!pangkalan) {
                throw new BadRequestException('Pangkalan tidak ditemukan');
            }

            // Block orders if pangkalan is inactive
            if (!pangkalan.is_active) {
                throw new BadRequestException('Pangkalan tidak aktif. Tidak dapat membuat pesanan.');
            }

            // Calculate this month's start and end (WIB timezone)
            const now = new Date();
            ...
            // Sum all 3kg orders for this pangkalan this month (excluding BATAL)
            ...
            // Sum all 3kg from penyaluran_harian this month
            ...
            // Use the higher of the two (to avoid double counting if order creates penyaluran)
        ...
        for (const item of dto.items) {
            if (item.lpg_product_id) {
                ...
                if (product && item.qty > currentStock) {
                    throw new BadRequestException(
                        `Stok ${product.name} ga cukup! Stok sekarang cuman: ${currentStock}`
                    );
                }
            }
        }
        ...
        // VOICE ORDER EXPRESS: Determine initial status
        // If is_voice_order=true && is_paid_cash=true → DIPROSES (skip DRAFT & MENUNGGU_PEMBAYARAN)
        // Otherwise → DRAFT (normal flow)
        ...
        const order = await this.prisma.orders.create({
            data: {
                ...
                current_status: initialStatus as any,
                order_items: {
        ...
        // VOICE ORDER EXPRESS: Auto-create payment record if paid cash
