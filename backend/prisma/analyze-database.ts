 ...
 * Mengecek semua tabel dan menganalisa mana yang kosong/belum terisi
 */
...
async function main() {
    ...
    for (const table of tables) {
        try {
            ...
            let status = '';
            if (count === 0) status = '❌ KOSONG';
            else if (count < 5) status = '⚠️  MINIM';
            else status = '✅ OK';
    ...
    if (paymentDetails === 0) console.log('   ⚠️  Tidak ada payment details untuk orders!');
