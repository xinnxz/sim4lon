/**
 * SEED: Laporan Demo Data
 * 
 * Creates realistic data for the Laporan page:
 * 1. Completed orders (SELESAI) for pangkalan transactions
 * 2. Consumers (registered subsidy recipients)
 * 3. Consumer orders (sales from pangkalan to end consumers)
 * 
 * This will populate:
 * - Total Transaksi, Total Tabung, Total Pendapatan
 * - Distribusi Subsidi 3kg section
 * - Ranking Pangkalan table
 * - Audit Konsumen Subsidi section
 * 
 * CARA PAKAI:
 * cd backend
 * npx ts-node prisma/seed-laporan.ts
 */
...
// Random Indonesian names for consumers
...
function generateNIK(): string {
    ...
    const tahun = Math.floor(Math.random() * 30 + 70).toString(); // 70-99
...
async function main() {
    ...
    // ============================================
    // CLEANUP: Delete previous laporan demo data
    // ============================================
    console.log('🧹 Cleaning up previous laporan demo data...');

    // Delete consumer orders (prefix PORD-L)
    ...
    // ============================================
    // 1. CREATE CONSUMERS (Subsidy Recipients)
    // ============================================
    console.log('\n👥 Creating consumers for each pangkalan...');

    let totalConsumersCreated = 0;
    const consumersByPangkalan: Record<string, any[]> = {};

    for (const pkl of pangkalans) {
        // Check existing consumers for this pangkalan
        const existingCount = await prisma.consumers.count({
            where: { pangkalan_id: pkl.id }
        });

        // Create 8-15 consumers per pangkalan if less than 5 exist
        ...
        for (let i = 0; i < toCreate; i++) {
            ...
            const isWarung = Math.random() > 0.7; // 30% warung
        ...
        // Get all consumers for this pangkalan
    ...
    // ============================================
    // 2. CREATE COMPLETED ORDERS (Pangkalan purchases)
    // ============================================
    ...
    for (const pkl of pangkalans) {
        // 3-8 orders per pangkalan this month
        ...
        for (let i = 0; i < numOrders; i++) {
            ...
            const qty = 15 + Math.floor(Math.random() * 35); // 15-50 tabung
    ...
    // ============================================
    // 3. CREATE CONSUMER ORDERS (Subsidy distribution)
    // ============================================
    ...
    for (const pkl of pangkalans) {
        ...
        // Each consumer buys 1-4 times this month
        for (const consumer of consumers) {
            ...
            for (let i = 0; i < numPurchases; i++) {
                ...
                // Warung can buy 3-5, rumah tangga 1-2
