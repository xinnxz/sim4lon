import{j as jsxRuntimeExports}from"./jsx-runtime.mvtouYz0.js";import{r as reactExports}from"./index.DcciWILN.js";import{S as SafeIcon,B as Button}from"./SafeIcon.ClRJUfEf.js";import{C as Card,a as CardContent}from"./card.DiHNBGLf.js";import{B as Badge}from"./badge.7K_czkF5.js";import{S as Separator}from"./separator.C67djOhB.js";import{T as Tabs,a as TabsList,b as TabsTrigger}from"./tabs.CRnBi62r.js";import{ordersApi,paymentApi,companyProfileApi}from"./api.BKkQ1phJ.js";import{f as formatCurrency}from"./currency.DQ9ceiwM.js";import{t as toast}from"./index.fAd_ikgp.js";import{u as useAppSettings}from"./useAppSettings.DDQwmAr3.js";import"./index.PIAPtHoh.js";import"./index.ubNQCTBn.js";import"./index.BwQOTkhO.js";import"./index.BB0xO63c.js";import"./index.thnc9KDl.js";import"./index.CSDUP7Pl.js";import"./index.Cn8uR79O.js";const getLpgImage=label=>{if(!label)return null;const l=label.toLowerCase();return l.includes("bright")||l.includes("220")?"/images/products/bright-gas-220gr.png":l.includes("50kg")||l.includes("50 kg")?"/images/products/lpg-50kg.png":l.includes("12kg")||l.includes("12 kg")?"/images/products/lpg-12kg.png":l.includes("5.5")||l.includes("5kg")||l.includes("5 kg")?"/images/products/lpg-5kg.png":l.includes("3kg")||l.includes("3 kg")?"/images/products/lpg-3kg.png":null};function NotaPembayaranPage(){const[documentType,setDocumentType]=reactExports.useState("nota"),[data,setData]=reactExports.useState(null),[isLoading,setIsLoading]=reactExports.useState(!0),[error,setError]=reactExports.useState(null),[isPrinting,setIsPrinting]=reactExports.useState(!1),[companyProfile,setCompanyProfile]=reactExports.useState(null),{settings:appSettings}=useAppSettings();reactExports.useEffect(()=>{if(typeof window>"u")return;const params=new URLSearchParams(window.location.search),orderId=params.get("code")||params.get("id")||params.get("orderId"),type=params.get("type");if((type==="invoice"||type==="nota")&&setDocumentType(type),!orderId){setError("Order ID tidak ditemukan"),setIsLoading(!1);return}fetchData(orderId)},[]);const fetchData=async orderId=>{try{setIsLoading(!0);const[order,payment,profile]=await Promise.all([ordersApi.getById(orderId),paymentApi.getOrderPayment(orderId).catch(()=>null),companyProfileApi.get().catch(()=>null)]);profile&&setCompanyProfile(profile);const pangkalan=order.pangkalans,items=order.order_items.map(item=>({name:item.label||`LPG ${item.lpg_type}`,quantity:item.qty,unitPrice:Number(item.price_per_unit),subtotal:Number(item.sub_total||item.price_per_unit*item.qty)})),subtotal=items.reduce((sum,item)=>sum+item.subtotal,0),taxAmount=items.reduce((sum,item)=>{const itemTax=order.order_items.find(oi=>(oi.label||oi.lpg_type)===item.name)?.tax_amount||0;return sum+Number(itemTax)},0),isPaidFromStatus=["DIPROSES","DIKIRIM","SELESAI"].includes(order.current_status),isPaid=payment?.is_paid||isPaidFromStatus;setData({orderId:order.id,orderCode:order.code||`ORD-${order.id.slice(0,4).toUpperCase()}`,orderDate:new Date(order.created_at).toLocaleDateString("id-ID",{year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit"}),customerName:pangkalan?.name||"Unknown",customerAddress:pangkalan?.address||"",customerPhone:pangkalan?.phone||"",customerEmail:pangkalan?.email||"",contactPerson:pangkalan?.pic_name||"",items,subtotal,taxRate:appSettings.ppnRate,taxAmount,total:order.total_amount,isPaid,paymentMethod:payment?.payment_method||null,paymentDate:payment?.payment_date?new Date(payment.payment_date).toLocaleDateString("id-ID"):null,amountPaid:Number(payment?.amount_paid||0),paymentProofUrl:payment?.proof_url||null}),setError(null)}catch(err){console.error("Failed to fetch document data:",err),setError(err.message||"Gagal memuat data dokumen")}finally{setIsLoading(!1)}},handlePrint=()=>{if(!data)return;setIsPrinting(!0);const isNota2=documentType==="nota",docNumber2=isNota2?`NOTA-${data.orderCode.replace("ORD-","")}`:`INV-${data.orderCode.replace("ORD-","")}`,originalTitle=document.title;document.title=`${isNota2?"Nota":"Invoice"}_${docNumber2}_${data.customerName.replace(/\s+/g,"_")}`,setTimeout(()=>{window.print(),document.title=originalTitle,setIsPrinting(!1)},100)},handleShareWhatsApp=()=>{if(!data)return;const isNota2=documentType==="nota",docTitle=isNota2?"NOTA PEMBAYARAN":"INVOICE",itemLines=data.items.map(item=>`• ${item.name} x${item.quantity} = ${formatCurrency(item.subtotal)}`).join(`
